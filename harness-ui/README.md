...
`uvicorn` come in with `--group dev`, and are also available as `--extra harness-ui` if you would
...
**Restart the server after changing anything under `src/fi/alk/harness/`.** A long-lived process
