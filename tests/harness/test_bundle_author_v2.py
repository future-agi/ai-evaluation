...
def test_contract_columns_wrapper_still_yields_declarations() -> None:
    """A table written as {"columns": {...}} carries the same hints as the flat shape."""
    wrapped = _contract_column_declarations(
        {
            "data_schema": {
                "intake_sessions": {
                    "columns": {
    ...
    assert wrapped == {
        ("intake_sessions", "session_id"): "TEXT PRIMARY KEY",
        ("intake_sessions", "ask_attempts"): "JSONB NOT NULL DEFAULT '{}'::jsonb",
    }


def test_contract_default_survives_a_baseline_that_declares_none(tmp_path: Path) -> None:
    """A NOT NULL column the source defaults must reach Postgres with that default.

    Without it the agent's own INSERT, which omits the column, is refused by the store and the
    tool answers 500.
    """
    authoring = _authoring(tmp_path)
    (authoring / "contract.json").write_text(
    ...
    try:
        database.execute(
            ...
            "session_id TEXT PRIMARY KEY, ask_attempts TEXT NOT NULL)"
        )
        database.execute("INSERT INTO intake_sessions VALUES (?, ?)", ("ses_1", "{}"))
        database.commit()
    finally:
        database.close()
    ...
    assert "\"ask_attempts\" jsonb NOT NULL DEFAULT '{}'" in compiled_world


def test_a_whole_create_table_string_still_yields_declarations() -> None:
    """The third shape the model writes: one statement per table rather than a mapping.

    Taken verbatim from a real contract, including the multi-line CHECK whose commas sit inside
    parentheses and the table-level constraint that is not a column.
    """
    declarations = _contract_column_declarations(
        {
            "data_schema": {
                "intake_sessions": (
                    ...
                    "                          CHECK (state IN ('in_progress','complete','ended_by_caller')),\n"
                    "    created_at            TIMESTAMPTZ NOT NULL DEFAULT now(),\n"
                    "    ask_attempts          JSONB NOT NULL DEFAULT '{}'::jsonb,\n"
                    "    UNIQUE (lead_id, state)\n"
                    ")"
                )
            }
        }
    )
    assert declarations[("intake_sessions", "ask_attempts")] == (
        "JSONB NOT NULL DEFAULT '{}'::jsonb"
    )
    assert declarations[("intake_sessions", "created_at")] == "TIMESTAMPTZ NOT NULL DEFAULT now()"
    assert "CHECK (state IN ('in_progress','complete','ended_by_caller'))" in (
        declarations[("intake_sessions", "state")]
    )
