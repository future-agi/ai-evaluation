...
class LocalDeepSeekClient:
    """Local DeepSeek-style client exposing the preset ``chat`` entry.

    Credential-free: no real deepseek import, no network. Returns a
    ...
    def chat(self, *, message=None, messages=None, model=None, **params):
        return {
            "content": "DeepSeek adapter approved refund with chat runtime evidence.",
            "tool_calls": [
                {
                    "id": "deepseek_status",
            ...
            "events": [
                {
                    ...
                    "name": "deepseek.chat",
                    "payload": {"framework": "deepseek", "model": model or "deepseek-chat"},
                }
            ],
            "state": {
                "provider_response": {
...
def run(output_path: str | Path) -> dict:
    result = simulate.run_framework_adapter_probe(
        "deepseek",
        LocalDeepSeekClient(),
        target="sdk_framework_adapter_cert_deepseek.py:LocalDeepSeekClient",
        method="chat",
        input_mode="dict",
        cases=[
            {
...
if __name__ == "__main__":
    run(Path("artifacts") / "sdk-framework-adapter-cert-deepseek.json")
