"""Consolidated promotion for the nested_method certification family.

Covers huggingface, whose preset entrypoint (``__call__``) returns a nested pipeline object the adapter coerces.
...
FAMILY = "nested_method"
FRAMEWORKS = ('huggingface',)
TARGET = f"{Path(__file__).resolve()}:LocalNestedMethodShim"

PRESET_METHODS = {'huggingface': ('__call__', 'dict')}
WEAK_METHOD = ('respond', 'text')



class LocalNestedMethodShim:
    """Local nested_method shim with a weak and a preset entrypoint."""
    ...
    def __call__(self, *, payload=None, **params: Any) -> dict[str, Any]:
        return {
...
def _evaluation_config(framework: str) -> dict[str, Any]:
    return {
        "task_description": (
            f"Promote the certified {framework} nested_method adapter into a "
...
def build_promotion(framework: str) -> dict[str, Any]:
    ...
    return optimize.optimize_framework_adapter_probe(
        ...
        framework=framework,
        target=TARGET,
        agent_factory=LocalNestedMethodShim,
        adapter_candidates=[
...
if __name__ == "__main__":
    destination = (
        Path(sys.argv[1])
        if len(sys.argv) > 1
        else Path("artifacts") / "sdk-framework-adapter-cert-nested-method-promotion.json"
    )
    run(destination)
