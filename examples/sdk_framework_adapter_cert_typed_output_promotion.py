"""Consolidated promotion for the typed_output certification family.

Covers instructor, whose preset entrypoint (``chat``) returns a typed/structured object.
...
TARGET = f"{Path(__file__).resolve()}:LocalTypedOutputShim"

PRESET_METHODS = {'instructor': ('chat', 'dict')}
WEAK_METHOD = ('respond', 'text')



class LocalTypedOutputShim:
    """Local typed_output shim with a weak and a preset entrypoint."""
...
def _evaluation_config(framework: str) -> dict[str, Any]:
    return {
        "task_description": (
            f"Promote the certified {framework} typed_output adapter into a "
...
def build_promotion(framework: str) -> dict[str, Any]:
    ...
    return optimize.optimize_framework_adapter_probe(
        ...
        framework=framework,
        target=TARGET,
        agent_factory=LocalTypedOutputShim,
        adapter_candidates=[
...
if __name__ == "__main__":
    destination = (
        Path(sys.argv[1])
        if len(sys.argv) > 1
        else Path("artifacts") / "sdk-framework-adapter-cert-typed-output-promotion.json"
    )
    run(destination)
