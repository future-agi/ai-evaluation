...
class LocalCerebrasClient:
    """Local Cerebras-style client exposing the preset ``chat`` entry.

    Credential-free: no real cerebras import, no network. Returns a
    ...
    def chat(self, *, message=None, messages=None, model=None, **params):
        return {
            "content": "Cerebras adapter approved refund with chat runtime evidence.",
            "tool_calls": [
                {
                    "id": "cerebras_status",
            ...
            "events": [
                {
                    ...
                    "name": "cerebras.chat",
                    "payload": {"framework": "cerebras", "model": model or "llama3.1-8b"},
                }
            ],
            "state": {
                "provider_response": {
...
def run(output_path: str | Path) -> dict:
    result = simulate.run_framework_adapter_probe(
        "cerebras",
        LocalCerebrasClient(),
        target="sdk_framework_adapter_cert_cerebras.py:LocalCerebrasClient",
        method="chat",
        input_mode="dict",
        cases=[
            {
...
if __name__ == "__main__":
    run(Path("artifacts") / "sdk-framework-adapter-cert-cerebras.json")
