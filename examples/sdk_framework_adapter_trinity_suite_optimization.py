...
class LocalRefundOrchestrator:
    """Local framework shim whose full trinity suite is optimized."""
    ...
    async def execute_task(self, payload: dict[str, Any]) -> dict[str, Any]:
        return {
            "content": (
                "Framework trinity suite optimization approved refund with "
...
def write_workspace(directory: str | Path) -> dict[str, Any]:
    return suite.write_framework_adapter_trinity_suite_optimization_workspace(
        ...
        metadata={"cookbook": "sdk-framework-adapter-trinity-suite-optimization"},
    )
...
def run(output_path: str | Path) -> dict[str, Any]:
    ...
    result["framework_adapter_trinity_optimization_workspace"] = workspace
...
if __name__ == "__main__":
    destination = (
        Path(sys.argv[1])
        if len(sys.argv) > 1
        else Path("artifacts") / "sdk-framework-adapter-trinity-suite-optimization.json"
    )
    run(destination)
