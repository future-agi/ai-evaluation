...
class LocalTogetherClient:
    """Local Together-style client exposing the preset ``chat`` entry.

    Credential-free: no real together import, no network. Returns a
    ...
    def chat(self, *, message=None, messages=None, model=None, **params):
        return {
            "content": "Together adapter approved refund with chat runtime evidence.",
            "tool_calls": [
                {
                    "id": "together_status",
            ...
            "events": [
                {
                    ...
                    "name": "together.chat",
                    "payload": {"framework": "together", "model": model or "meta-llama/Llama-3-8b-chat-hf"},
                }
            ],
            "state": {
                "provider_response": {
...
def run(output_path: str | Path) -> dict:
    result = simulate.run_framework_adapter_probe(
        "together",
        LocalTogetherClient(),
        target="sdk_framework_adapter_cert_together.py:LocalTogetherClient",
        method="chat",
        input_mode="dict",
        cases=[
            {
...
if __name__ == "__main__":
    run(Path("artifacts") / "sdk-framework-adapter-cert-together.json")
