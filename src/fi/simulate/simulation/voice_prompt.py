...
def format_voice_persona(
    ...
    *,
    ...
    # No example closing here on purpose. A quoted line is spoken back verbatim: the previous
    # wording supplied one and it turned up word for word in transcripts, and it was English, which
    # is wrong the moment the caller speaks another language. Describe the behaviour only.
    rules_section += "13. **Call Closing:** Let the agent finish speaking before you end the call. When the conversation has concluded, or the other side has closed it, say your own brief closing once in your own words and call the endCall tool in the same turn. Never speak the words 'function', 'tool' or 'endCall' aloud, and never announce that you are ending the call. Do not answer a closing with another closing, and do not leave the call open once you are finished.\n"
