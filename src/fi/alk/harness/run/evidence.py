...
def newest_report(started: float, root: Path | None = None) -> dict[str, Any]:
    ...
    """
    search_root = Path(root) if root is not None else ACCEPTANCE
    if not search_root.exists():
        return {}
    newest: tuple[float, Path] | None = None
    for report in search_root.glob("run_*/*/report.json"):
...
def tracks_in(case: dict[str, Any]) -> list[dict[str, str]]:
    ...
    for artifact in (case.get("metadata") or {}).get("provider_artifacts") or []:
        ...
        if artifact.get("type") == "audio" and path and Path(path).exists():
            found.append(
                {
                    "label": f"{artifact.get('artifact_id', 'provider')}",
                    "path": str(path),
                }
            )
    return found
...
def spoken_times(case: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    starts = [
        m.get("started_speaking_at") for m in messages if m.get("started_speaking_at")
    ]
    if not starts:
        return []
...
def measured(case: dict[str, Any]) -> dict[str, Any]:
    ...
    report = (
        (case.get("evaluation") or {}).get("agent_report")
        or metadata.get("agent_report_summary")
        or {}
    )
