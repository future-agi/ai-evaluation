...
describe('Templates', () => {
    describe('Template Structure', () => {
        ...
        it('should have unique eval_ids across all templates', () => {
            const canonicalTemplates = Array.from(new Set(Object.values(Templates)));
            const evalIds = canonicalTemplates.map(t => t.eval_id).filter(Boolean);
        ...
        it('should have unique eval_names across all templates', () => {
            const canonicalTemplates = Array.from(new Set(Object.values(Templates)));
            const evalNames = canonicalTemplates.map(t => t.eval_name);
    ...
    describe('Tone & Style Templates', () => {
        ...
        it('should alias NoOpenAIReference to NoLLMReference', () => {
            expect(Templates.NoOpenAIReference).toBeDefined();
            expect(Templates.NoOpenAIReference).toBe(Templates.NoLLMReference);
            expect(Templates.NoOpenAIReference.eval_id).toBe('');
            expect(Templates.NoOpenAIReference.eval_name).toBe('no_llm_reference');
        });
    });
    ...
    describe('Format & Structure Templates', () => {
        ...
        it('should not expose removed IsCSV template', () => {
            expect(Templates.IsCSV).toBeUndefined();
        });

        it('should not expose removed IsCode template', () => {
            expect(Templates.IsCode).toBeUndefined();
        });
        ...
        it('should not expose removed NoValidLinks template', () => {
            expect(Templates.NoValidLinks).toBeUndefined();
        });
    });
    ...
    describe('Specialized Templates', () => {
        ...
        it('should alias LLMFunctionCalling to EvaluateFunctionCalling', () => {
            expect(Templates.LLMFunctionCalling).toBeDefined();
            expect(Templates.LLMFunctionCalling).toBe(Templates.EvaluateFunctionCalling);
            expect(Templates.LLMFunctionCalling.eval_id).toBe('98');
            expect(Templates.LLMFunctionCalling.eval_name).toBe('evaluate_function_calling');
        });

        it('should alias AudioTranscriptionEvaluator to ASRAccuracy', () => {
            expect(Templates.AudioTranscriptionEvaluator).toBeDefined();
            expect(Templates.AudioTranscriptionEvaluator).toBe(Templates.ASRAccuracy);
            expect(Templates.AudioTranscriptionEvaluator.eval_id).toBe('');
            expect(Templates.AudioTranscriptionEvaluator.eval_name).toBe('ASR/STT_accuracy');
        });
    ...
    describe('Hallucination Templates', () => {
        it('should alias DetectHallucinationMissingInfo to DetectHallucination', () => {
            expect(Templates.DetectHallucinationMissingInfo).toBeDefined();
            expect(Templates.DetectHallucinationMissingInfo).toBe(Templates.DetectHallucination);
            expect(Templates.DetectHallucinationMissingInfo.eval_id).toBe('');
            expect(Templates.DetectHallucinationMissingInfo.eval_name).toBe('detect_hallucination');
        });
    ...
    describe('Other Templates', () => {
        ...
        it('should not expose removed NotGibberishText template', () => {
            expect(Templates.NotGibberishText).toBeUndefined();
        });

        it('should not expose removed SafeForWorkText template', () => {
            expect(Templates.SafeForWorkText).toBeUndefined();
        });
    ...
    describe('Template Count', () => {
        it('should have the current revamped template and alias catalog', () => {
            const templateCount = Object.keys(Templates).length;
            expect(templateCount).toBe(75);
        });
    });
